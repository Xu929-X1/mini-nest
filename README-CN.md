...
- 🌳 **Trie 路由** - 基于前缀树的快速路由匹配，支持参数、通配符（`*`）和贪婪通配符（`**`）
...
class FileController {
    @Get('/assets/*')
    serveAsset() {
        // 匹配 /api/assets/logo.png、/api/assets/style.css
    }

    @Get('/files/*/download')
    downloadFile() {
        // 匹配 /api/files/report.pdf/download
    }

    @Get('/proxy/**')
    proxyRequest() {
        // 匹配 /api/proxy/v1/users/123/posts — 任意深度
    }
}
```

每段匹配优先级：**静态 > 参数（`:id`）> 通配符（`*`）> 贪婪通配符（`**`）**

> `**` 必须位于路径末尾 — `@Get('/api/**/users')` 启动时抛出异常。
